...
const agents_1 = require("@climateshield/agents");
const index_js_1 = require("./index.js");
async function main() {
    const { plan, state, persistedTo } = await (0, index_js_1.runIncidentResponse)(agents_1.incidentFixtureINC204, {
        onEvent: (e) => console.log(`  • ${e.stage.padEnd(16)} ${e.status.padEnd(10)} (attempts=${e.attempts}, fallback=${e.usedFallback})`),
    });
    ...
    console.log(`Confidence: ${plan.confidence} (<= engine ${agents_1.incidentFixtureINC204.baseRisk.confidence})`);
    ...
    for (const a of plan.dispatch.recommendedActions) {
        console.log(`  [${a.priority.toUpperCase()}] ${a.actionId}` +
