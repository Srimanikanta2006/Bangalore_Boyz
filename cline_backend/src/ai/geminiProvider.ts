...
import type { ExplainRequest, ExplainResponse } from "./schemas";
import { validateExplainResponse } from "./validation";

/**
 * Gemini provider for the ClimateShield explanation layer.
 *
 * Grounding + safety guarantees:
 *  - Low temperature, JSON-only output.
 *  - The model receives ONLY verified engine facts (see explainAdapter.ts).
 *  - Every response is runtime-validated (validation.ts) and constrained
 *    (incidentId must match; confidence cannot exceed the engine's confidence).
 *  - ANY failure (missing key, HTTP error, timeout, bad JSON, invalid schema)
 *    falls back to a deterministic explanation. AI is never a single point of failure.
 */
