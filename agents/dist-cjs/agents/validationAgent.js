...
class ValidationAgent {
    ...
    integrate(input) {
        ...
        const warnings = [...input.priorWarnings];
        const { facts } = input;
        ...
        for (const action of input.dispatch.recommendedActions) {
            if (!(0, actionCatalog_1.isValidActionId)(action.actionId)) {
                warnings.push(`Dropped non-catalog action "${action.actionId}".`);
                continue;
            }
            if (!(0, actionCatalog_1.isPriorityAllowed)(action.actionId, action.priority)) {
                warnings.push(`Dropped action "${action.actionId}" with disallowed priority "${action.priority}".`);
                continue;
            }
            if (action.targetAssetCode && !validAssetCodes.has(action.targetAssetCode)) {
                warnings.push(`Dropped action "${action.actionId}" targeting unknown asset "${action.targetAssetCode}".`);
                continue;
            }
            let suggestedUnitCallsign = action.suggestedUnitCallsign;
            if (suggestedUnitCallsign && !validCallsigns.has(suggestedUnitCallsign)) {
                warnings.push(`Cleared unknown unit "${suggestedUnitCallsign}" from action "${action.actionId}".`);
        ...
        for (const dep of input.dispatch.actionDependencies) {
            if (presentActionIds.has(dep.actionId) && presentActionIds.has(dep.dependsOnActionId)) {
                ...
                warnings.push(`Dropped dependency ${dep.actionId} -> ${dep.dependsOnActionId} (missing action).`);
            }
        }
        // 3. Global confidence ceiling = engine confidence.
        const engineConfidence = (0, util_1.clamp01)(facts.baseRisk.confidence);
        let confidence = (0, util_1.clamp01)(input.risk.confidence);
        if (confidence > engineConfidence) {
            warnings.push(`Capped confidence ${confidence} to engine confidence ${engineConfidence}.`);
