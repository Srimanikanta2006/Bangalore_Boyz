...
exports.ACTION_CATALOG = void 0;
exports.isValidActionId = isValidActionId;
exports.getCatalogAction = getCatalogAction;
exports.isPriorityAllowed = isPriorityAllowed;
exports.actionsForAssetType = actionsForAssetType;
exports.catalogPromptBlock = catalogPromptBlock;
exports.ACTION_CATALOG = [
    {
...
const CATALOG_BY_ID = new Map(exports.ACTION_CATALOG.map((a) => [a.id, a]));
function isValidActionId(actionId) {
    return CATALOG_BY_ID.has(actionId);
}
function getCatalogAction(actionId) {
    return CATALOG_BY_ID.get(actionId);
}
function isPriorityAllowed(actionId, priority) {
...
function actionsForAssetType(assetType) {
    return exports.ACTION_CATALOG.filter((a) => a.suitedAssetTypes.includes(assetType));
}
/** The catalog rendered for an LLM prompt (id + allowed priorities). */
function catalogPromptBlock() {
    return exports.ACTION_CATALOG.map((a) => `- ${a.id} (allowed priorities: ${a.allowedPriorities.join(", ")}) — ${a.description}`).join("\n");
}
