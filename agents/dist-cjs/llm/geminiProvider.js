...
exports.GeminiProvider = exports.DEFAULT_GEMINI_MODEL = void 0;
...
class GeminiProvider {
    ...
    async complete(prompt, options = {}) {
        ...
        try {
            const res = await this.fetchFn(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(this.model)}:generateContent`, {
                method: "POST",
            ...
            if (controller.signal.aborted) {
                throw new Error(`Gemini request timed out after ${(options.timeoutMs ?? DEFAULT_TIMEOUT_MS) / 1000}s.`);
            }
 ...
 */
function createDefaultProvider(config = {}) {
    const provider = new GeminiProvider(config);
    if (provider.available)
        return provider;
    return new provider_1.NullLlmProvider();
}
