...
export function register(name: string, email: string, password: string, role?: string): Promise<LoginResult> {
  return api.post<LoginResult>('/auth/register', { name, email, password, role }, { anonymous: true });
}
